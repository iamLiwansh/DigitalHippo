...
const NavItem = ({category,handleOpen,isAnyOpen,isOpen}: NavItemProps) => {
    return (<div className="flex">
        ...
        {isOpen ? (<div className={cn("absolute inset-x-0 top-full text-sm text-muted-foreground",{
            'animate-in fade-in-10 slide-in-from-top-5': !isAnyOpen,//condtion lagyi hai agar !isAnyOpen false hoga too hi apply ho
        })}>
                    ...
                    <div className="grid grid-cols-4 gap-x-8 gap-y-10 py-16">
                        <div className="col-span-4 col-start-1 grid grid-cols-3 gap-x-8">
                            {category.featured.map((item) => (
                                <div key={item.name} className="group realtive text-base sm:text-sm">
