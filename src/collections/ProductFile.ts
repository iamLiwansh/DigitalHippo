...
const yourOwnAndPurchased : Access = async ({req}) => {
    ...
    const { docs: products} = await req.payload.find({
        collection: "products",
        depth: 0,//its  mean that only id will be fetched .. ye SQL mai JOIN ki trah hai agar ham depth ko 1 kr denge too entire user info fetch ho jayegi
        where: {
            user : {
                equals: user.id,
            },
    ...
    const { docs: orders} = await req.payload.find({
        collection: "orders",
        depth: 2, //it will give the user info 
        where: {
            user : {
                equals: user.id,
            },
